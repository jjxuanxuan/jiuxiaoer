...
func collectMetrics(db *gorm.DB) []metrics.Sample {
	...
	samples := make([]metrics.Sample, 0, len(counts)+8)
	...
	samples = append(samples,
		metrics.Sample{Name: "jxe_refund_oldest_pending_seconds", Help: "Age of the oldest active refund.", Type: "gauge", Value: oldestAge(db, "refunds", "status IN ('creating','submission_unknown','pending') AND deleted_at IS NULL", "requested_at", now)},
		metrics.Sample{Name: "jxe_refund_oldest_submission_unknown_seconds", Help: "Age of the oldest refund whose submission result is unknown.", Type: "gauge", Value: oldestAge(db, "refunds", "status='submission_unknown' AND deleted_at IS NULL", "requested_at", now)},
	...
	var terminalCounts []statusCount
	_ = db.Table("refunds").Select("provider_status AS status, COUNT(*) count").Where("provider_status IN ('CLOSED','ABNORMAL') AND deleted_at IS NULL").Group("provider_status").Scan(&terminalCounts).Error
	for _, row := range terminalCounts {
		samples = append(samples, metrics.Sample{Name: "jxe_refund_provider_terminal_tasks", Help: "Refunds requiring CLOSED or ABNORMAL recovery.", Type: "gauge", Labels: map[string]string{"provider_status": row.Status}, Value: float64(row.Count)})
	}
